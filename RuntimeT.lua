...
local AIP = false
local AddonHandler = require(script.Parent.Parent["Addon Handler"])()
...
if(loaded_conf["Running"]~=nil)then SE("easyTannoy Build 2 :: Another instance was found running, this instance will not work (another script detected in this instance).","warn")end
loaded_conf.Setup()
...
local Runtime_L = script.Parent:WaitForChild("Runtime_L",5) or script
wait(0.2)
local Runtime_L_Event = Runtime_L:FindFirstChild("Node")
local newEvent = Instance.new("BindableFunction",event)
if Runtime_L_Event ~= nil then
	newEvent:Remove()
else
	newEvent.Name = "Node"
	Runtime_L_Event = newEvent
	spawn(function()
		newEvent.OnInvoke = function(_,_)
			return true
		end
	end)
end
if loaded then
	...
	event.OnServerInvoke = function(plr,info)
		...
		if info["Type"] == "PlaySound" then
			if wh(plr) == true then
				if loaded_conf.Cooldown == false then AIP = false end
				if AIP then return end
				AIP = true
				...
				spawn(function()
					AddonHandler.Function("SoundPlayed",{
						plr,
						sound_button.Name,
						info["Tannoy"],
					})
				...
				local function play()
					...
					sound:Play()
					sound.Ended:Wait()
					AddonHandler.Function("SoundEnded",{
						info["Tannoy"]
					})
					AIP = false
				end
		...
		if info["Type"] == "Login"  then
			if wh(plr) == true and info["Tannoy"]:GetAttribute("USER_LOGGED_IN") == false and Runtime_L_Event:Invoke(plr,"OnLogin") == true then
				AddonHandler.Function("LoggedIn",{
					plr,
					info["Tannoy"]
				})
		...
		if info["Type"] == "Logout" then
			if wh(plr) == true and info["Tannoy"]:GetAttribute("USER_LOGGED_IN") == true then
				AddonHandler.Function("LoggedOut",{
					info["Tannoy"]
				})
				Runtime_L_Event:Invoke(plr,"Logout")
